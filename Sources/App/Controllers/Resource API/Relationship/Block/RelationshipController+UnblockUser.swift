...
extension RelationshipController {
    public func unblockUserHandler(_ req: Request) async throws -> AnyAsyncResponse {
        ...
        switch version {
        case .v1:
            let result = try await unblockUserV1(req)
            return .init(result)
        default:
            throw Abort(.notFound)
        }
    ...
    private func unblockUserV1(_ req: Request) async throws -> VNVCECore.Relationship.V1 {
        ...
        guard let blockID = relationship.blockID,
              let block = try await Block.find(blockID, on: req.db)
        else {
